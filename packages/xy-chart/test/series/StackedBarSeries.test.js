...
describe('<StackedBarSeries />', () => {
  ...
  it('should render one rect per x value per stack key', () => {
    const wrapper = shallow(
      <XYChart {...mockProps}>
        <StackedBarSeries stackKeys={mockStackKeys} data={mockData} />
      </XYChart>,
  ...
  it('should use stackFills for color', () => {
  ...
  it('should call onMouseMove({ datum, data, event, seriesKey, color }), onMouseLeave(), and onClick({ datum, data, event, seriesKey, color }) on trigger', () => {
    const fills = ['magenta', 'maplesyrup', 'banana'];
