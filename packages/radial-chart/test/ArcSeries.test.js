...
describe('<ArcSeries />', () => {
  ...
  it('should render an Arc', () => {
    const wrapper = mount(
      <ArcSeries pieValue={d => d.value} data={[{ value: 10 }, { value: 5 }]} label={null} />,
    );
    expect(wrapper.find(Arc)).toHaveLength(1);
  });

  it('should render an Arc for slices and an Arc for Labels', () => {
    const wrapper = mount(
      <ArcSeries
        pieValue={d => d.value}
    ...
    expect(wrapper.find(Arc)).toHaveLength(2);
  });

  it('should pass arc objects to the label accessor and use the output of the accessor', () => {
    const wrapper = mount(
      <ArcSeries
        pieValue={d => d.value}
        data={[{ value: 10 }, { value: 5 }]}
  ...
  it('should call onMouseMove({ datum, data, fraction, event }) and onMouseLeave() on trigger', () => {
